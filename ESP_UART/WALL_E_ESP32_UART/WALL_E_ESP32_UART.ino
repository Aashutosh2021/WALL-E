...
// NOTE: deliberately NOT using the board's silkscreen RX2/TX2 (GPIO16/17) —
// those are already DIR_CLK/DIR_EN above (shift register). Using custom
// free pins instead; ESP32 lets any GPIO be assigned to a hardware UART.
#define PI_RX_PIN  4    // ESP32 RX  <- wire to Raspberry Pi GPIO14 (TXD, physical pin 8)
#define PI_TX_PIN  27   // ESP32 TX  -> wire to Raspberry Pi GPIO15 (RXD, physical pin 10)
...
void executeCommand(String cmd) {
  ...
  if (cmd == "EYES_TALKING" || cmd == "SPEAK") {
    ...
    // Unmatched command — likely UART corruption/noise. Log it as REJECTED,
    // not "Executed", so real problems are visible instead of hidden.
    appendLog("REJECTED unknown/corrupt command -> '" + cmd + "'");
    return;
  }
...
void setup() {
  // Serial0 (USB) — programming/debug ONLY now. Pi link moved to Serial2.
  Serial.begin(115200);

  // Hardwired UART2 to Raspberry Pi (GPIO4=RX, GPIO27=TX — see defines above)
  Serial2.begin(9600, SERIAL_8N1, PI_RX_PIN, PI_TX_PIN);
  Serial2.setTimeout(50);  // 1000ms block hatakar 50ms fast timeout
  Serial2.setRxBufferSize(4096);  // handle large base64 image strings safely
...
void loop() {
  ...
  // Read incoming commands from Raspberry Pi (hardwired UART2, not USB)
  if (Serial2.available() > 0) {
    String incomingCmd = Serial2.readStringUntil('\n');
    incomingCmd.trim();

    if (incomingCmd.length() > 0) {
      appendLog("UART2 Rx: Received command -> '" + incomingCmd + "'");
      executeCommand(incomingCmd);

      // ACK Back to Pi
      Serial2.println("ACK_" + incomingCmd);
    }
  }
}
...
- **Master ("The Brain"):** Raspberry Pi 3B+ (or Pi 4B) running Raspberry Pi OS Lite (64-bit). Manages full-duplex WebSocket streaming to Gemini Live API, persistent camera frame capture (`picamera2`), asynchronous tool execution, SQLite WAL conversation memory, and local audio streaming.
- **Slave ("The Muscle & Display"):** ESP32 Dev Module running C++ Arduino firmware. Controls 4 DC motors using a 74HC595 shift register with hardware PWM speed control and dynamic per-command auto-stop, and hosts a standalone Wi-Fi Soft-AP web server for live CSS OLED eye expressions, camera previews, and debug logging.
...
| **Slave Controller** | ESP32-WROOM-32 Dev Module | 5.0V (VIN) / 3.3V Logic | 250mA | UART Serial (`/dev/serial0` or `/dev/ttyUSB0`) |
| **Camera Sensor** | OV5647 5MP CSI Pi Camera | 3.3V (via Ribbon) | 250mA | MIPI-CSI2 / `picamera2` |
| **Motor Driver** | L293D Dual H-Bridge Motor Shield | 5.0V (Logic) / 6.0V (Motors) | 2.0A Peak | 74HC595 Shift Register + 4x PWM Channels |
...
| **Voltage Regulators** | 2x LM2596 Step-Down Buck Modules | In: 11.1V -> Out: 5.0V & 6.0V | 3.0A per module | Common Ground Reference |
| **Audio Input** | USB Plug-and-Play Microphone | 5.0V (USB) | 50mA | ALSA / `sounddevice` (16kHz PCM) |
| **Audio Output** | USB / 3.5mm Speaker with Amplifier | 5.0V (USB) | 500mA | PortAudio / `sounddevice` (24kHz Native PCM) |
...
- **Core Libraries:**
  ...
  - `pyserial` (Persistent USB/UART serial communication with ESP32 and background reader)
  - `sqlite3` (WAL-mode persistent conversation logging and context formatting)
...
- **Core Libraries:**
  ...
  - Hardware PWM (`ledcAttach` / `ledcWrite`) for 4-channel motor speed control
...
├── SQLite Database Memory Footprint:         ~5 MB
└── Active Audio Ingest/Playback Buffers:     ~10 MB
TOTAL RUNNING RAM USAGE:                      ~135 MB
FREE / CACHED RAM:                            ~785 MB (Zero Swapping)
```
...
| :--- | :--- | :--- |
| **Mic Ingest (32ms / 64ms Chunks)** | < 70ms | 32ms – 64ms buffer |
...
| **Vision REST API Call (Ollama / Gemini Flash)**| < 1200ms | ~350ms – 900ms |
| **UART Command Dispatch** | < 5ms | ~1ms – 2ms |